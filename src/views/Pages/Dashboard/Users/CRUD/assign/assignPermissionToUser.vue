    ...
    >
      <template v-slot:activator="{ on, attrs }">
        <div class="delete__dialog__delete">
          ...
          <v-btn
              ...
              v-bind="attrs"
              v-on="on"
              @click="pullUserPermissions(item)"
          >
      ...
      <v-card class="delete__card">
              ...
              <v-col cols="12" md="5">
                ...
                <div class="permission__border">
                  <div @click="selectItem(permission,1,index)" :class="'permission__item'+' '+permission.customClass"
                       v-for="(permission,index) in getAllUserPermission" :key="permission.id">
              ...
              <v-col cols="12" md="5">
                <div class="create__user">
                  <v-text-field
                      ...
                      outlined
                      dense
                      v-model="searchUserPermission"
                  ></v-text-field>
                </div>
                <div class="permission__border">
                  <div @click="selectItem(permission,2,index)" :class="'permission__item'+' '+permission.customClass"
                       v-for="(permission,index) in getUserUserPermissions" :key="permission.id">
        ...
        <v-card-actions class="delete__card__actions">
          ...
          <div class="delete__dialog__delete">
            <v-btn @click="assignPermissionToUser">
              Assign
            </v-btn>
          </div>
...
export default {
  ...
  watch:
      {
        searchUserPermission(newValue) {
          this.$store.commit('authorization/searchUserUserPermission', newValue)

        },
        searchPermission(newValue) {
          this.$store.commit('authorization/searchUserPermission', newValue)

        },
        getDialogCloseAssignUserPermission(newValue) {
  ...
  computed: {
    ...mapGetters("authorization", [
      ...
      "getDialogCloseAssignUserPermission",
    ]),

  },
  methods: {
    assignPermissionToUser() {
      this.$store.dispatch('authorization/assignPermissionToUser')
    },
    pullUserPermissions(item) {

      this.$store.dispatch('authorization/pullUserPermissions', item.id)
    },
    selectItem(item, check, index) {
      this.selectedItem.check = check;
      this.selectedItem.item = item;
      this.selectedItem.index = index;
      this.$store.commit('authorization/selectUserPermission', this.selectedItem)

    },
    unselectPermission() {
      this.$store.commit('authorization/unselectUserPermission', this.selectedItem)

    },
    movingPermission() {
      this.$store.commit('authorization/movingUserPermission', this.selectedItem)
